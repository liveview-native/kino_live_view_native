...
defmodule KinoLiveViewNativeTest.ExampleLive do
  ...
  test "start server and register a LiveView" do
    ...
    # KinoLiveViewNative.register({:module, KinoLiveViewNativeTest.ExampleLive, 0, 0}, "/", ":index")

    # assert Req.get!("http://localhost:4000").body =~ "Hello Web"
    assert IO.inspect(Req.get!("http://localhost:4000?_lvn[format]=swiftui")).body =~ "Hello Native"
  end
end
